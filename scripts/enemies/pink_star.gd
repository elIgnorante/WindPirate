...
func _physics_process(delta):
        ...
        if not attacking and not recovering and patrol_distance > 0:
                if is_on_wall():
                        _flip_direction()
...
func _update_movement_animation():
        ...
        if not is_on_floor():
                ...
                if patrol_distance > 0 and abs(velocity.x) > 5.0:
